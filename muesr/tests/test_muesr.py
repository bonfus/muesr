...
class TestMuesr(unittest.TestCase):
    ...
    def test_one_dipole_outside(self):
        ...
        r = locfield(m, 's',[1,1,1],8.66/2.)
        self.assertEqual( np.sum(np.abs(r.D[0])), 0.)
    ...
    def test_cubic_sym1(self):
        ...
        m.mm = mm
        m.add_muon([0.5,0.5,0.5])
        r = locfield(m, 's',[100,100,100],240)
        # Bdip should be: [0 tesla, 11.4226 milliteslas, 11.4226 milliteslas]
        #
        #    (magnetic_constant/4pi)⋅((1/(1angstrom⋅sqrt(2.5^2+2.5^2+2.5^2)))^3)⋅(3⋅(1 bohr_magneton)⋅cos(atan(sqrt(2)))⋅([1,1,1]/sqrt(3))−1bohr_magneton⋅[1,0,0])
        #
    ...
    def test_rotation(self):
        ...
        r = locfield(m, 'r',[1,1,1],1.2,nnn=0,nangles=300,axis=[1,0,0])
        ...
        #print('AAAAAAAAAAAAAAAAA', r.D.shape)
        ...
        r = locfield(m, 'r',[5,1,1],1.5,nnn=0,nangles=300,axis=[1,0,0])
        np.testing.assert_array_almost_equal(r.D[0],-r.D[150],decimal=7)
        ...
        m.mm = mm
        i = locfield(m, 'i',[1,1,1],1.2,nnn=0,nangles=300)
        ...
        i = locfield(m, 'i',[5,1,1],1.5,nnn=0,nangles=300)
    ...
    def test_double_moment(self):
        ...
        r = locfield(m, 's',[1,1,1],4)
        m.mm.fc_set(np.array([[0.+0j,0.+0.j,2.+0.j]]))
        r2 = locfield(m, 's',[1,1,1],4)
        # r should be: [0 tesla, 11.4226 milliteslas, 11.4226 milliteslas]
        #
        #    (magnetic_constant/4pi)⋅((1/(1angstrom⋅sqrt(2.5^2+2.5^2+2.5^2)))^3)⋅(3⋅(1 bohr_magneton)⋅cos(atan(sqrt(2)))⋅([1,1,1]/sqrt(3))−1bohr_magneton⋅[1,0,0])
        #
    ...
    def test_compare_ass_and_rass(self):
        ...
        min_oom = min(self.oom(r1.T[0,0]), self.oom(r2.T[0,0]))
        np.testing.assert_array_almost_equal(r1.T[0,0],r2.T[0,0],decimal=6-min_oom)

        min_oom = min(self.oom(r1.T[1]), self.oom(r2.T[1,0]))
        np.testing.assert_array_almost_equal(r1.T[1,0],r2.T[1,0],decimal=6-min_oom)
    ...
    def test_compare_rass_and_incass(self):
        ...
        min_oom = min(self.oom(r1.T[0]), self.oom(r2.T[0]))
        np.testing.assert_array_almost_equal(r1.T[0],r2.T[0,0],decimal=7-min_oom)
        min_oom = min(self.oom(r1.T[1]), self.oom(r2.T[1,0]))
        np.testing.assert_array_almost_equal(r1.T[1],r2.T[1,0],decimal=7-min_oom)

        r2_norms = np.apply_along_axis(np.linalg.norm, 1, r2.D[0])
        r3_norms = np.apply_along_axis(np.linalg.norm, 1, r3.D[0])
        ...
        r2_norms = np.apply_along_axis(np.linalg.norm, 1, r2.T[1])
        r3_norms = np.apply_along_axis(np.linalg.norm, 1, r3.T[1])
    ...
    @unittest.skipIf(have_pymatgen == False, 'PyMatGen not available')
    def test_magmodel_input(self):
        ...
        nmm = MM(m._cell.get_number_of_atoms(),m._cell.cell.array)
